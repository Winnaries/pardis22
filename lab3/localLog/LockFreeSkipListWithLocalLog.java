...
public final class LockFreeSkipListWithLocalLog<T> {
	/* Number of levels */
		...
		while (true) {
			...
			try {
				if (!pred.next[bottomLevel].compareAndSet(succ, newNode, false, false))
					continue;
				book.get().record(1, x, true, start, "@" + topLevel + " " + pred.key);
			} finally {
			}
	...
	public boolean remove(T x, ThreadLocal<LockFreeSkipListRecordBook<T>> book) {
		...
		while (true) {
			boolean found = find(x, preds, succs);
			if (!found) {
				book.get().record(2, x, false, start, "non-existent");
				return false;
			}
			...
			while (true) {
				...
				if (iMarkedIt) {
					...
					book.get().record(2, x, true, start, "by itself");
					find(x, preds, succs);
					return true;
				} else if (marked[0]) {
					// NOTE: Other node get ahead of this node, 
					// and proceed to remove the node first. 
					book.get().record(2, x, false, start, "by others");
					return false;
				}
	...
	public boolean contains(T x, ThreadLocal<LockFreeSkipListRecordBook<T>> book) {
		...
		for (int level = MAX_LEVEL; level >= bottomLevel; level--) {
			...
			while (true) {
				...
				if (!found && !marked[0]) {
					if (curr.key == v) {
						book.get().record(0, x, true, start, "found @" + level + " " + pred.key);
						found = true;
					} else if (level == bottomLevel && curr.key > v) {
						book.get().record(0, x, false, start, "not found @" + level + " " + pred.key + " -> " + curr.key);
						found = true;
					}
				}
